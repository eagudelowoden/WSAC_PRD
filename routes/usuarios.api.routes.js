...
// Listar segmentos
router.get("/segmentos/lista", (req, res) => {
  try {
    if (!fs.existsSync(RUTA_SEGMENTOS)) return res.json([]);
    const segmentos = fs
...
router.get("/cargos/:segmento", async (req, res) => {
  try {
...
router.delete("/:id", registrarActividad("Eliminar usuario"), (req, res) => {
  const {
