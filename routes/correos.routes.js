...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ status: "error", message: "Error al procesar documentos" });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    const { id } = req.body;
    db.query(
      "SELECT nombres, correo, cargo FROM usuarios WHERE id = ?",
      [id],
      async (err, users) => {
        if (err || !users?.length)
          return res
...
router.post(
  ...
  async (req, res) => {
    const { id } = req.body;
    db.query(
      "SELECT nombres, apellidos, cargo, salario, segmento_contrato, ciudad FROM usuarios WHERE id = ?",
      [id],
      async (err, users) => {
        if (err || !users?.length)
          return res
        ...
        db.query(
          ...
          async (errN, resN) => {
            if (errN || !resN?.length)
              return res
                .status(404)
                .json({ status: "error", message: "Sin correos de nómina" });

            try {
              await correoOutlook.sendMail({
                from: `"WSAC Sistema" <${process.env.OUTLOOK_USER}>`,
                to: resN.map((r) => r.email),
  ...
  async (req, res) => {
    ...
    db.query(
      ...
      async (err, users) => {
        ...
        const u = users[0];
        db.query("SELECT email FROM notificaciones", async (errN, resN) => {
          if (errN || !resN?.length)
            return res
              .status(404)
              .json({ status: "error", message: "Sin correos configurados" });
